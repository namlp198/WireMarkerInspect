...
Session update — matching diagnostics and independent result colors 2026-09-04:
- Found the all-zero bug: featureCandidates discarded intermediate counts on early gate failures, leaving a default NoCandidate result. Native now preserves descriptor/filter/inlier counts, separate verification reason and availability flags; diagnostic appearance may continue after a failed feature gate only when geometry permits. The failed gate still forces NG; failed second-attempt diagnostics cannot replace a valid first candidate.
- Additive result diagnostics retain all thresholds and distinguish unmeasured N/A from measured zero. Neither recipe schema/values nor matching/OCR acceptance gates were relaxed. Invalid polygons are not presented as valid match outlines.
- RUN and teaching show independent green/red checks for text, orientation, template and numeric thresholds. Actual text is 18 DIP; check details 16 DIP. Critical parameters are marked amber with localized impact hints and promoted out of the advanced group. Small RUN panels scroll instead of clipping HUDs.
- Read-only replay of saved cycle d37ae96e234c43c491c13578f32dbe0e, model4/v8 (saved recipe equals cycle recipe): SIFT end1 38 matches/24 inliers, ratio .63158 below .65; NCC .98895, SSIM .91150, edge/score .69364. End2 12 matches/4 inliers with invalid transform. AKAZE on these same frames: end1 70/57, score .77249 below .8; end2 4/4 with inadequate matches and invalid geometry. These are diagnostic observations, not labeled production-accuracy acceptance.
- Added read-only tools/MatchingReplay and seven regression cases (124 managed total). Both native configurations and managed suites pass; real WPF mixed-color/template/profile/role smoke passes. Final artifact paths are recorded in handoff/latest.md. No hardware or PLC writes; no user recipes/results modified. Changes remain uncommitted.

Session update — Debug native matching export fix 2026-09-04:
- Export inspection confirmed Debug/build and Debug/Desktop DLLs lacked `wmi_matching_abi_version`/`wmi_match`; both Release DLLs had them. Prior matching verification had only rebuilt Release.
- Desktop/F5 now runs incremental native CMake for the active configuration before collecting runtime files. A dedicated item type preserves DLL filenames during MSBuild metadata batching. WPF temporary/design-time projects skip native builds; full build scripts skip the redundant nested invocation after building native themselves.
- Matcher checks all required exports with TryGetExport before calling the version function, returning actionable rebuild/restart guidance for outdated modules. Added regression coverage for deployed exports and missing-export diagnostics.
- Verification: Debug/Release clean builds; 117/117 managed and 1/1 native in both configurations; WPF smoke PASS Debug `artifacts/smoke-20260904-163344`, Release `artifacts/smoke-20260904-163449`. Debug source/output DLL SHA-256 matched. No recipe, inspection thresholds or hardware logic changed; no hardware commands. Uncommitted.

Session update — terminal template matching 2026-09-04:
- Implemented all five approved per-end algorithms: Normal multi-pose matching, AKAZE, SIFT, ORB and ORB Max Stable. Strict geometry/valid-mask checks precede independent NCC/local SSIM/edge/combined-score gates; ambiguous candidates reject. No fallback or expected-text-conditioned processing.
- New recipe schema v3 stores immutable template PNGs, learn/search ROIs and separate stable per-algorithm profiles. New models enable matching by default; v1/v2 retain explicitly labeled OCR-only behavior. RUN copies all assets/config and combines terminal result with the existing exact text/direction checks on the same frame; Simulator shares that path.
- Admin-only template modal uses shared ImageHud editors, independent template/test loading, basic/advanced thresholds, test diagnostics and aligned previews. Cancel leaves the end unchanged; Apply End and Save remain explicit. RUN adds template/aligned/source-outline evidence. Language switches preserve every profile value.
- Verification: Release 0 warnings/errors; managed 115/115; native 1/1; real WPF smoke PASS at `artifacts/smoke-20260904-155757`. Synthetic matching fixtures exercise all five algorithms, wrong terminal/angle/reflection, duplicate candidates, masks, asset validation, immutable persistence, combined verdict/cancellation, default/legacy behavior and localization.
- Real-terminal accuracy/false accepts and target-PC cycle time are not yet validated. The old 26-image OCR dataset remains incomplete. No hardware or PLC writes. See `docs/architecture/TERMINAL_MATCHING.md` for teaching, defaults, native contract and limitations. Changes are uncommitted.
